...
x.store.StoreTree = x.store.Store.clone({
    id : "StoreTree",
    root_doc_id: "root",
    root_doc_title: "Everything",
    create_properties: { keyPath: "uuid" },
    indexes: [
        { id: "by_title" , key_path: "payload.title", additional: { unique: true } },
        { id: "by_parent", key_path: [ "payload.parent_id", "payload.sequence_nbr" ], additional: { unique: false } }
    ]
});


x.store.StoreTree.defbind("getOrCreateRootDoc", "start", function () {
    var that = this;
	...
	that.getDoc(that.root_doc_id)
		...
		.then(null, /* catch */ function (/*reason*/) {
		    ...
		    that.root_doc = { uuid: that.root_doc_id, payload: { title: that.root_doc_title, type: "folder" } };
...
x.store.StoreTree.define("setParent", function (node_id, parent_id, position) {
    var that = this,
        old_parent_id;
...
x.store.StoreTree.define("reset", function () {
    var that = this;
