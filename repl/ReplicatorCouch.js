...
x.repl.ReplicatorCouch = x.repl.ReplicatorBase.clone({
    id: "CouchReplicator"
});


x.repl.ReplicatorCouch.define("instantiate", function (id, server_url) {
   if (this.instance) {
        this.throwError("can't instantiate on an instance");
    }
    return this.clone({
        id: id,
         local_store: x.store.StoreIndexedDB.clone({ id: id + "StoreIdxDB", db_id: id.toLowerCase(), instance: true, store_id: "main", version: 3 }),
        remote_store: x.store.StoreCouch    .clone({ id: id + "StoreCouch", db_id: id.toLowerCase(), instance: true, server_url: server_url })
    });
});


x.repl.ReplicatorCouch.defbind("startLocalStore", "start", function () {
    return this. local_store.start();
});


x.repl.ReplicatorCouch.defbind("startRemoteStore", "start", function () {
    return this.remote_store.start();
});


//use changelog?
x.repl.ReplicatorCouch.override("getServerDocChanges", function () {
    var that = this;
